	    ...
	    <div class="home-content">
	      <i class='bx bx-menu'></i>
			...
			foreach($produto as $p){
				...
				$esp = listarCaracProdEsp($conexao,$idprod);
				foreach ($esp as $value) {
					if($value['IdProduto'] == $idprod){
						echo '<td>R$ '.$value["Valor_Bruto"].'.00</td>';
						echo '<td>R$ '.$value["Valor_Vendido"].'.00</td>';
					}
				}
